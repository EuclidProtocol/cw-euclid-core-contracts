...
impl KeyDeserialize for Token {
    type Output = Token;
    /// `key()` emits a single segment, so a composite key splitter must consume exactly one.
...
impl KeyDeserialize for Pair {
    type Output = Pair;
    /// `key()` emits three segments (the token_1 size prefix, then each token).
...
impl TokenType {
    ...
    /// Decimals-agnostic equality: two tokens are shallow-equal when they are
    /// the same variant and refer to the same denom / contract address,
    /// regardless of the `decimals` field. A given (chain, token) denom is only
    /// ever registered with a single decimals value, so ignoring decimals here
    /// is unambiguous — and it lets callers that don't know the decimals (e.g. a
    /// cw20 `Send` hook building `decimals: None`) still match a registered
    /// denom. Use this for allow-list / membership checks.
    pub fn shallow_eq(&self, other: &TokenType) -> bool {
        match (self, other) {
            ...
            (TokenType::Voucher {}, TokenType::Voucher {}) => true,
            _ => false,
        }
    }

    /// Full structural equality, including the `decimals` field (equivalent to
    /// the derived `PartialEq`). Named explicitly to contrast with
    /// [`TokenType::shallow_eq`] at call sites where the decimals distinction
    /// must be preserved.
    pub fn deep_eq(&self, other: &TokenType) -> bool {
        self == other
    }
...
mod tests {
    ...
    #[rstest]
    // Same denom, differing decimals → shallow-equal but not deep-equal.
    #[case(
        TokenType::Native { denom: "uatom".to_string(), decimals: Some(6) },
        TokenType::Native { denom: "uatom".to_string(), decimals: None },
        true,
        false
    )]
    #[case(
        TokenType::Smart { contract_address: "cosmos1abc".to_string(), decimals: Some(18) },
        TokenType::Smart { contract_address: "cosmos1abc".to_string(), decimals: None },
        true,
        false
    )]
    // Identical decimals → equal both ways.
    #[case(
        TokenType::Native { denom: "uatom".to_string(), decimals: Some(6) },
        TokenType::Native { denom: "uatom".to_string(), decimals: Some(6) },
        true,
    ...
    #[case(TokenType::Voucher {}, TokenType::Voucher {}, true, true)]
    // Different denom → unequal both ways.
    #[case(
        TokenType::Native { denom: "uatom".to_string(), decimals: None },
        TokenType::Native { denom: "uosmo".to_string(), decimals: None },
        false,
        false
    )]
    // Different variant, same string → unequal both ways.
    #[case(
        TokenType::Native { denom: "abc".to_string(), decimals: None },
        TokenType::Smart { contract_address: "abc".to_string(), decimals: None },
        false,
        false
    )]
    fn test_token_type_eq_variants(
        #[case] a: TokenType,
        #[case] b: TokenType,
        #[case] shallow: bool,
        #[case] deep: bool,
    ) {
        assert_eq!(a.shallow_eq(&b), shallow, "shallow_eq");
        assert_eq!(b.shallow_eq(&a), shallow, "shallow_eq is symmetric");
        assert_eq!(a.deep_eq(&b), deep, "deep_eq");
        assert_eq!(a.deep_eq(&b), a == b, "deep_eq matches derived PartialEq");
    }
}
