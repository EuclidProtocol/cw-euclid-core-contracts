...
fn test_add_liquidity_fails_with_zero_liquidity_amount() {
    ...
    // The zero leg goes only into the add-liquidity request; the pool above is
    // created with valid amounts so this exercises the add-liquidity guard.
    let mut zero_pair_info = pair_info;
    zero_pair_info.token_1.amount = Uint256::zero();

    // adding funds (skip the zero leg: a zero-amount coin is not sendable)
    let chain = interchain
        .get_chain(factory.environment().chain_id().as_str())
        .unwrap();
    let mut funds = vec![];
    for token in zero_pair_info.get_vec_token_info() {
