...
//! `RouterCrossChainExecuteMsg` variant. Used to exercise the defence-in-depth
...
pub fn execute(
    ...
    // Pack a `RouterCrossChainExecuteMsg::RegisterDenom` (non-pool variant)
    // into a `PoolFactoryReply::SendPacket`. Main factory's reply handler MUST
    // reject this packet before dispatching anything.
    let evil_packet = RouterCrossChainExecuteMsg::RegisterDenom {
