...
- [concentrated_vlp] CLP swap execution and single-hop `SimulateSwap` apply `euclid_fee_override` by lowering the structural fee tier for the swap (new `resolve_effective_fee` helper), so a reduced/zero override actually improves the trader's quote instead of only shifting the protocol's cut to LPs; the LP keeps its absolute pip share of the tier (`lp_pips = tier_pips - tier_pips*d/10_000`) and only the protocol's slice (`tier_pips*X/10_000`) is waived; an override at/above the pool default is an exact no-op. Multi-hop forwarding to downstream legs is deferred to Slice 2 (SC-23 Issue 8)
