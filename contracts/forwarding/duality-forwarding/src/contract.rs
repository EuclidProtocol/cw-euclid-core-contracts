...
use crate::{
    ...
    reply::{on_duality_swap_reply, DUALITY_SWAP_REPLY_ID},
    state::{State, STATE},
};

use forwarding::msgs::duality::{ExecuteMsg, InstantiateMsg, QueryMsg};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:duality-forwarding";
...
pub fn instantiate(
    ...
    let state = State {
        duality_router_address: msg.duality_router.clone(),
    };
    STATE.save(deps.storage, &state)?;
    set_contract_version(deps.storage, CONTRACT_NAME, CONTRACT_VERSION)?;
    Ok(Response::new()
        .add_attribute("method", "instantiate")
        .add_attribute("duality_router", msg.duality_router))
}
...
pub fn reply(deps: DepsMut, env: Env, msg: Reply) -> Result<Response, ContractError> {
    match msg.id {
        DUALITY_SWAP_REPLY_ID => on_duality_swap_reply(deps, env, msg),
