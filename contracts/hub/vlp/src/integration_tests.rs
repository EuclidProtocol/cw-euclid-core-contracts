#[cfg(test)]
mod tests {
    use crate::{contract, state::{State, STATE}};

    use super::*;
    use cosmwasm_std::{Addr, Coin, Empty, Uint128};
    use cw_multi_test::{App, AppBuilder, Contract, ContractWrapper, Executor};
    use euclid::{fee::Fee, msgs::vlp::{AllPoolsResponse, ExecuteMsg, FeeResponse, GetLiquidityResponse, GetSwapResponse, InstantiateMsg, PoolResponse, QueryMsg}, token::{Pair, PairInfo, Token, TokenInfo, TokenType}};
    ...
    fn test_instantiate_contract() {
        ...
        let instantiate_msg = InstantiateMsg {
            ...
            pair: Pair {
                token_1: Token { id: "token_1_address".to_string() },
                token_2: Token { id: "token_2_address".to_string() },
            },
            fee: Fee {
                lp_fee: 1,
                treasury_fee: 1,
    ...
    fn test_register_pool() {
        ...
        let register_msg = ExecuteMsg::RegisterPool {
            ...
            pair_info: PairInfo {
                token_1: TokenInfo {
                    token: Token { id: "token_1_address".to_string() },
                    token_type: TokenType::Native { denom: "token_1".to_string() },
                },
                token_2: TokenInfo {
                    token: Token { id: "token_2_address".to_string() },
        ...
        let res: PoolResponse = app
            .wrap()
            .query_wasm_smart(contract_addr.clone(), &QueryMsg::Pool { chain_id: "chain_1".to_string() })
            .unwrap();

        assert_eq!(res.pool.chain, "chain_1".to_string());
        assert_eq!(res.pool.pair.token_1.token.id, "token_1_address".to_string());
        assert_eq!(res.pool.pair.token_2.token.id, "token_2_address".to_string());
    }
    ...
    fn test_query_simulate_swap() {
        ...
        let simulate_swap_msg = QueryMsg::SimulateSwap {
            asset: Token { id: "token_1".to_string() },
