...
pub const VOUCHER_DECIMAL: u8 = 24;
...
pub const VOUCHER_ALLOWANCES: Map<SerializedBalanceKey, VoucherAllowance> = Map::new("voucher_allowances");

// Token Metadata is stored as a map of token id to token metadata.
pub const TOKEN_METADATA: Map<(String, ChainUid, &str), TokenMetadata> = Map::new("token_metadata");
...
pub fn get_token_metadata_key(
    ...
    TOKEN_METADATA.key((token, chain_uid, token_type.get_key().as_str()))
}

// Escrow balances are stored as Uint256 to avoid precision loss.
pub const ESCROW_BALANCES: Map<(String, ChainUid, &str), Uint256> = Map::new("escrow_balances");
...
pub fn get_escrow_balance_key(
    ...
    ESCROW_BALANCES.key((token, chain_uid, token_type.get_key().as_str()))
}