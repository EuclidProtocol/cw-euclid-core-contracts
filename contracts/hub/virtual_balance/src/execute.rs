...
pub fn execute_normalize_balance_keys(
    ...
    for result in VOUCHER_ALLOWANCES
        .range(deps.storage, None, None, Order::Ascending)
        .skip(skip)
        .take(limit)
    {
    ...
    for (key, allowance) in allowance_entries {
        ...
        if lowercase_address != address {
            ...
            if let Some(existing) =
                VOUCHER_ALLOWANCES.may_load(deps.storage, normalized_key.clone())?
            {
                ...
                VOUCHER_ALLOWANCES.save(deps.storage, normalized_key, &allowance)?;
            }
...
mod tests {
    ...
    fn test_normalize_allowance_keys_keeps_max_on_collision() {
        ...
        assert!(VOUCHER_ALLOWANCES.load(&deps.storage, mixed_key).is_err());
        let result = VOUCHER_ALLOWANCES.load(&deps.storage, lower_key).unwrap();
        assert_eq!(result.amount, Uint256::from(300u128));
    }
