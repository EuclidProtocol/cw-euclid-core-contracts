...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // Branch 2 – LegacyStateWithEuclidAdmin exists → admin is migrated
    // -------------------------------------------------------------------------

    fn write_legacy_state_with_euclid_admin(
        deps: &mut cosmwasm_std::OwnedDeps<
            cosmwasm_std::testing::MockStorage,
            cosmwasm_std::testing::MockApi,
            cosmwasm_std::testing::MockQuerier,
    ...
    #[test]
    fn test_migrate_promotes_legacy_euclid_admin_to_separate_state() {
        let mut deps = mock_dependencies();
        let env = mock_env();

        let admin_addr = deps.api.addr_make("admin");
        let euclid_admin = EuclidAdmin::default(admin_addr);
        write_legacy_state_with_euclid_admin(&mut deps, euclid_admin.clone());
        ...
        assert_eq!(saved_state.router, Addr::unchecked("router"));
    }

    // -------------------------------------------------------------------------
    // Branch 3 – LegacyState (String admin) exists → admin is migrated
    // -------------------------------------------------------------------------
