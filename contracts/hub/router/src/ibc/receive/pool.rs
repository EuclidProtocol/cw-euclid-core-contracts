...
use euclid_ibc::wire::msgs::{
    ...
    RemoveLiquiditySendMsg, SingleSidedAddLiquiditySendMsg,
};
...
pub fn ibc_execute_add_concentrated_liquidity(
    mut deps: DepsMut,
    msg: AddConcentratedLiquiditySendMsg,
) -> Result<Response, ContractError> {
    ...
    for token in msg.pair.get_vec_token_info() {
        ...
        // A zero leg (one-sided concentrated add) mints and approves nothing;
        // the virtual balance contract rejects zero-amount operations.
...
pub fn ibc_execute_remove_concentrated_liquidity(
    deps: DepsMut,
    _env: Env,
    msg: RemoveConcentratedLiquiditySendMsg,
) -> Result<Response, ContractError> {
    let vlp_address = CONCENTRATED_VLPS.load(deps.storage, msg.pool_key.to_map_key())?;
    let response = Response::new()
        .add_event(tx_event(
...
pub fn ibc_execute_collect_concentrated_fees(
    deps: DepsMut,
    _env: Env,
    msg: CollectConcentratedFeesSendMsg,
) -> Result<Response, ContractError> {
    let vlp_address = CONCENTRATED_VLPS.load(deps.storage, msg.pool_key.to_map_key())?;
    let response = Response::new()
        .add_event(tx_event(
...
pub fn ibc_execute_collect_concentrated_protocol_fees(
    deps: DepsMut,
    _env: Env,
    msg: CollectConcentratedProtocolFeesSendMsg,
) -> Result<Response, ContractError> {
    let vlp_address = CONCENTRATED_VLPS.load(deps.storage, msg.pool_key.to_map_key())?;
    let response = Response::new()
        .add_event(tx_event(
...
pub fn ibc_execute_single_sided_add_liquidity(
    deps: DepsMut,
    _env: Env,
    msg: SingleSidedAddLiquiditySendMsg,
) -> Result<Response, ContractError> {
    msg.pair.validate()?;
...
mod tests {
    ...
    use euclid_ibc::wire::{
        ...
        msgs::{AddLiquiditySendMsg, RemoveLiquiditySendMsg, RequestPoolCreationSendMsg},
    };
    ...
    fn make_remove_liquidity_msg(chain_uid: &ChainUid, tx_id: &str) -> RouterReceiveMsg {
        let pair = Pair::new(
    ...
    fn test_ibc_remove_liquidity_duplicate_tx_fails(mut initialized: MockDeps) {
        ...
        .unwrap();
        let pending = RemoveLiquiditySendMsg {
            sender: CrossChainUser::new(chain_uid.clone(), "user".to_string()),
            lp_allocation: Uint256::from(100u128),
            pair,
            recipient: CrossChainUser::new(chain_uid.clone(), "recipient".to_string()),
    ...
    mod clp_normalization {
        ...
        use euclid_ibc::wire::msgs::AddConcentratedLiquiditySendMsg;
