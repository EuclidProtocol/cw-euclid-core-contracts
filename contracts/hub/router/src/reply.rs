use cosmwasm_std::{
    ensure, from_json, to_json_binary, DepsMut, Env, Reply, Response, SubMsg, SubMsgResult, WasmMsg,
};
...
use euclid_ibc::{
    ...
    wire::envelope::{factory::FactoryReceiveMsg, make_ack_fail, AcknowledgementMsg},
    wire::transcode,
};
        ...
        SubMsgResult::Ok(result) => {
                ...
                from_json::<ConcentratedPoolCreationResponse>(data.clone())
            {
                ...
                let response = ibc_execute_add_concentrated_liquidity(
                    deps,
                    euclid_ibc::wire::msgs::AddConcentratedLiquiditySendMsg {
...
pub fn on_reply_native_ibc_wrapper_call(
    ...
    let original_msg: FactoryReceiveMsg = from_json(original_packet.original_msg)?;
...
pub fn on_cross_chain_receive_reply(deps: DepsMut, msg: Reply) -> Result<Response, ContractError> {
    ...
    match msg.result.clone() {
        SubMsgResult::Err(err) => {
            ...
            let wire_ack = transcode::router_ack_json_to_wire(ctx.wire_tag, &ack_json, encoding)
            ...
            // side is the event's source_port. The event's destination is
            // the incoming packet's source, so the `destination_chain_type`
            // attribute is fed from `source_chain_type`.
        ...
        SubMsgResult::Ok(res) => {
            ...
            let wire_ack = transcode::router_ack_json_to_wire(ctx.wire_tag, &data, encoding)
                .map_err(map_encoding_err)?;
...
mod tests {
    ...
    use euclid_ibc::wire::msgs::{RemoveLiquiditySendMsg, SwapSendMsg};
    ...
    const IN_FLIGHT_SOURCE_PORT: &str = "chain1.factory1";
    const IN_FLIGHT_DESTINATION_PORT: &str = "vsl.router1";
    const IN_FLIGHT_MSG: &[u8] = b"in-flight-wire-msg";

    fn seed_in_flight_receive(deps: &mut MockDeps, sequence: u128, encoding: u8, wire_tag: u8) {
    ...
    #[test]
    fn test_cross_chain_receive_reply_ok_emits_payload_event() {
        let mut deps = initialized();
        seed_in_flight_receive(&mut deps, 7, 0, 12);
        ...
        assert_eq!(
            res.data,
            Some(cosmwasm_std::Binary::from(ack_bytes.to_vec()))
        );
    ...
    #[test]
    fn test_cross_chain_receive_reply_ok_abi_leg_transcodes_ack() {
        use euclid_encoding::Encoding;
        use euclid_ibc::wire::envelope::AcknowledgementMsg;
        use euclid_ibc::wire::msgs::SwapAckMsg;
        use euclid_ibc::wire::transcode;

        let mut deps = initialized();
        // wire_tag 12 = TAG_SWAP, encoding 1 = Abi.
        seed_in_flight_receive(&mut deps, 11, 1, 12);

        let ack_json = cosmwasm_std::to_json_binary(&AcknowledgementMsg::Ok(SwapAckMsg {
            amount_out: Uint256::from(800u128),
            tx_id: "tx-swap".to_string(),
        }))
        .unwrap();
        let proto_bytes = encode_execute_response(&ack_json);
        let reply = ok_reply(CROSS_CHAIN_RECEIVE_REPLY_ID, proto_bytes);
        ...
        let wire = cosmwasm_std::Binary::new(
            euclid_encoding::repr::from_transport_string(&ack_attr, Encoding::Abi)
                .expect("ack must be 0x hex"),
        );
        let back = transcode::router_ack_wire_to_json(12, &wire, Encoding::Abi).unwrap();
    ...
    #[test]
    fn test_cross_chain_receive_reply_err_emits_error_payload_event() {
        let mut deps = initialized();
        seed_in_flight_receive(&mut deps, 9, 0, 12);
        ...
        let expected_fail =
            euclid_ibc::wire::envelope::make_ack_fail("receive error".to_string()).unwrap();
        assert_eq!(
    ...
    #[test]
    fn test_cross_chain_receive_reply_without_in_flight_context_errors() {
        let mut deps = initialized();
        let proto_bytes = encode_execute_response(b"ack_payload");
        let reply = ok_reply(CROSS_CHAIN_RECEIVE_REPLY_ID, proto_bytes);
        assert!(on_cross_chain_receive_reply(deps.as_mut(), reply).is_err());
    }
    ...
    fn seed_pending_remove_liquidity(deps: &mut MockDeps, tx_id: &str) {
        ...
        .unwrap();
        let msg = RemoveLiquiditySendMsg {
            sender: CrossChainUser::new(
                ChainUid::create("chain1".to_string()).unwrap(),
                "user1".to_string(),
            ),
    ...
    mod single_sided {
        ...
        use euclid_ibc::wire::msgs::SingleSidedAddLiquiditySendMsg;
        ...
        fn seed_pending(deps: &mut MockDeps, pending: &SingleSidedAddLiquiditySendMsg) {
