...
use euclid_ibc::{
    wire::envelope::factory::FactoryReceiveMsg,
    wire::msgs::RegisterFactorySendMsg,
    wire::{
...
use euclid::{
    admin,
    chain::{Chain, ChainUid, CosmosChain, EvmChain, TvmChain},
...
pub fn execute_register_factory(
    ...
    match chain_info {
        ...
        RegisterFactoryChainType::Tvm(tvm_info) => {
            // TVM (TRON) mirrors EVM: 20-byte lowercase 0x-hex factory address.
            ensure!(
                tvm_info.factory_address.to_lowercase() == tvm_info.factory_address,
                ContractError::new("Factory address must be lowercase")
            );
            let chain = Chain {
                chain_uid: chain_uid.clone(),
                factory_address: tvm_info.factory_address,
                chain_type: euclid::chain::ChainType::Tvm(TvmChain {
                    chain_id: tvm_info.factory_chain_id,
                }),
            };
            Ok(response.add_submessage(msg.to_msg(
                deps,
...
pub fn execute_meta_receive(
    ...
    let data: RouterReceiveMsg = from_json(msg.call_data.clone())?;
    match data {
        RouterReceiveMsg::Swap(swap_msg) => {
            process_swap_meta_transaction(deps, env, info, sender, swap_msg)
        }
        RouterReceiveMsg::TransferVoucher(transfer_voucher_msg) => {
...
fn process_transfer_voucher_meta_transaction(
    ...
    info: MessageInfo,
    sender: CrossChainUser,
    mut transfer_voucher_msg: TransferVoucherSendMsg,
) -> Result<Response, ContractError> {
    ensure!(
...
mod tests {
    ...
    use euclid_ibc::wire::{
        envelope::router::RouterReceiveMsg,
        msgs::{RegisterDenomSendMsg, SwapSendMsg},
    };
            ...
            None => {
                ...
                match &res.messages[0].msg {
                    CosmosMsg::Wasm(WasmMsg::Execute {
                        ...
                        // FactoryReceiveMsg::RegisterFactory.
                        ...
                        } = cb
                        {
                            ...
                            let factory_msg: FactoryReceiveMsg = from_json(inner).unwrap();
                            // Verify chain identity; tx_id includes the bech32 sender
                            // address generated by addr_make so we assert it separately.
                            if let FactoryReceiveMsg::RegisterFactory(RegisterFactorySendMsg {
    ...
    fn meta_receive_msg(verified_sender: CrossChainUser, inner: &RouterReceiveMsg) -> ExecuteMsg {
    ...
    fn test_meta_receive_unsupported_msg_type_fails(mut initialized: MockDeps) {
        ...
        let inner = RouterReceiveMsg::RegisterDenom(RegisterDenomSendMsg {
            sender: sender.clone(),
    ...
    fn test_meta_receive_swap_requires_voucher_asset_in(mut initialized: MockDeps) {
        ...
        let res = execute(
            ...
            mock_env(),
            message_info(&meta_tx, &[]),
            meta_receive_msg(sender, &RouterReceiveMsg::Swap(swap)),
        );
        assert_eq!(
            res.unwrap_err(),
            ContractError::new("Asset IN does not match asset OUT")
    ...
    fn test_meta_receive_swap_sender_address_mismatch_fails(mut initialized: MockDeps) {
        ...
        let res = execute(
            ...
            meta_receive_msg(real_sender, &RouterReceiveMsg::Swap(swap)),
        );
        assert_eq!(
            res.unwrap_err(),
            ContractError::new("Sender address mismatch")
