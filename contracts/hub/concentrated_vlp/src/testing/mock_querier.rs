...
use cosmwasm_std::{coin, BalanceResponse, BankQuery, QuerierWrapper, Uint128};
use cosmwasm_std::{
    from_json,
...
impl WasmMockQuerier {
    pub fn handle_query(&self, request: &QueryRequest<cosmwasm_std::Empty>) -> QuerierResult {
        match &request {
            ...
            //         _ => panic!("Unsupported smart query"),
            //     }
            // }
    ...
    fn handle_create_denom(&self, msg: &Binary) -> QuerierResult {
        match from_json(msg).unwrap() {
            MsgCreateDenom { sender, subdenom } => {
                let res = MsgCreateDenomResponse {
