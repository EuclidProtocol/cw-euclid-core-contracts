...
pub fn execute_meta_transaction(
    ...
    let derived_address = match chain_type {
        ...
        // TVM (TRON) uses the same secp256k1 + keccak256 signing and the same 20-byte hex
        // address derivation as EVM (Base58Check is a client-side display encoding only), so
        // it shares the EVM verification path.
        ChainType::Evm(_) | ChainType::Tvm(_) => {
