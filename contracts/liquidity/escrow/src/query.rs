...
pub fn query_token_allowed(deps: Deps, denom: TokenType) -> Result<Binary, ContractError> {
    ...
    let response = AllowedTokenResponse {
        // Verify decimals-agnostically: a (chain, token) denom is only ever
        // registered with one decimals value, and callers (e.g. a cw20 `Send`
        // hook) don't always know it, so matching on type + denom is sufficient.
        allowed: registered_denom
            .iter()
            .any(|registered| registered.shallow_eq(&denom)),
    };
...
mod tests {
    ...
    #[rstest]
    fn test_query_build_info(initialized: MockDeps) {
        use euclid::build_info::BuildInfoResponse;
        let res: BuildInfoResponse =
            from_json(query(initialized.as_ref(), mock_env(), QueryMsg::GetBuildInfo {}).unwrap())
                .unwrap();
        assert_eq!(res.contract_version, env!("CARGO_PKG_VERSION"));
    ...
    /// Regression: a denom registered with explicit decimals must still be
    /// allowed when queried with `decimals: None` (and any other decimals).
    /// The cw20 `Send` hook sends `decimals: None`; verification is now
    /// decimals-agnostic so the deposit is no longer rejected.
    #[rstest]
    #[case(None)]
    #[case(Some(6))]
    #[case(Some(18))]
    fn test_query_token_allowed_ignores_decimals(
        mut initialized: MockDeps,
        #[case] query_decimals: Option<u32>,
    ) {
        let factory = initialized.api.addr_make("factory");
        let info = message_info(&factory, &[]);
        // Register a smart denom with explicit decimals.
        execute(
            initialized.as_mut(),
            mock_env(),
            info,
            ExecuteMsg::AddAllowedDenom {
                denom: TokenType::Smart {
                    contract_address: "cw20token".to_string(),
                    decimals: Some(18),
                },
            },
        )
