...
pub fn execute_request_register_denom(
    ...
    let request_register_denom_msg = RouterReceiveMsg::RegisterDenom(RegisterDenomSendMsg {
        token: token.clone(),
        sender,
        tx_id: tx_id.clone(),
    })
...
pub fn execute_request_deregister_denom(
    ...
    let request_deregister_denom_msg = RouterReceiveMsg::DeregisterDenom(DeregisterDenomSendMsg {
        token: token.clone(),
        sender,
        tx_id: tx_id.clone(),
    })
...
pub fn execute_deposit_token(
    ...
    let deposit_token_msg = RouterReceiveMsg::DepositToken(DepositTokenSendMsg {
        sender,
        asset_in,
        amount_in,
        tx_id: tx_id.clone(),
...
pub fn execute_transfer_voucher(
    ...
    // Reject an empty recipients list synchronously, matching the Solidity
    // factory's `if (recipients.length == 0) revert`. Without this the packet
    // is accepted and reaches the hub, where the recipient loop never runs and
    // the transfer is a no-op success ack — a cross-VM divergence in rejection
    // surface (see testing/euclid-tests voucher transfer tests).
    ensure!(
        !recipients.is_empty(),
        ContractError::new("Recipients cannot be empty")
    );
    let state = STATE.load(deps.storage)?;
    for recipient in recipients.iter() {
        recipient.validate()?;
    ...
    let chain_type = get_chain_type(deps.as_ref(), &env)?;
    let withdraw_msg = RouterReceiveMsg::TransferVoucher(TransferVoucherSendMsg {
        sender,
        token: token_id,
        amount,
        from,
...
mod tests {
    ...
    #[test]
    fn test_transfer_voucher_empty_recipients_fails() {
        let mut deps = mock_dependencies();
        init(&mut deps);
