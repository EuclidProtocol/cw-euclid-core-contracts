...
use euclid_ibc::wire::{
    ...
    msgs::{
        AddConcentratedLiquiditySendMsg, CollectConcentratedFeesSendMsg,
        CollectConcentratedProtocolFeesSendMsg, RemoveConcentratedLiquiditySendMsg,
        RemoveLiquiditySendMsg, RequestConcentratedPoolCreationSendMsg,
...
fn validate_pool_creation_tokens(
    ...
    for token in tokens {
        ...
        // Vouchers skip the FundManager path below, which is what implicitly
        // rejects zero for native/smart tokens, so the guard must be explicit
        // and cover every token type.
        ...
        ContractError::new(
            "Cannot create pool with two new tokens. At least one token must already be registered."
        )
    );
...
pub fn execute_request_pool_creation(
    ...
    let pool_create_msg = RouterReceiveMsg::RequestPoolCreation(RequestPoolCreationSendMsg {
        sender,
        tx_id: tx_id.clone(),
...
pub fn add_liquidity_request(
    ...
    let add_liq_msg = RouterReceiveMsg::AddLiquidity(AddLiquiditySendMsg {
        sender,
        slippage_tolerance_bps,
        pair: pair_info,
        tx_id: tx_id.clone(),
...
pub fn remove_liquidity_request(
    ...
    let remove_liq_msg = RouterReceiveMsg::RemoveLiquidity(RemoveLiquiditySendMsg {
        sender,
        lp_allocation,
        pair,
        recipient,
...
pub fn add_concentrated_liquidity_request(
    ...
    let mut fund_manager = FundManager::new(&info.funds);
    let tokens = pair_info.get_vec_token_info();
    // A concentrated position entirely on one side of the current price needs
    // only that side's token, so a single zero leg is legal here (unlike pool
    // creation and classic add). Both legs zero is still rejected.
    ensure!(
        tokens.iter().any(|token| !token.amount.is_zero()),
    ...
    let chain_type = get_chain_type(deps.as_ref(), &env)?;
    let add_liq_msg = RouterReceiveMsg::AddConcentratedLiquidity(AddConcentratedLiquiditySendMsg {
        sender,
        pair: pair_info,
        pool_key,
        lower_tick_index,
...
pub fn collect_concentrated_fees_request(
    ...
    let chain_type = get_chain_type(deps.as_ref(), &env)?;
    let collect_msg = RouterReceiveMsg::CollectConcentratedFees(CollectConcentratedFeesSendMsg {
        sender,
        pool_key,
        position_id,
        recipient,
...
pub fn execute_single_sided_add_liquidity_request(
    ...
    let ibc_msg = RouterReceiveMsg::SingleSidedAddLiquidity(SingleSidedAddLiquiditySendMsg {
        sender,
        asset_in: asset_in.clone(),
        amount_in,
        swap_amount,
...
mod tests {
    ...
    use crate::{
        contract::execute,
        state::PENDING_CONCENTRATED_ADD_LIQUIDITY,
        testing::helpers::{
    ...
    // -----------------------------------------------------------------------
    // Execute: AddConcentratedLiquidity – zero-amount legs
    // -----------------------------------------------------------------------

    fn clp_pool_key() -> euclid::msgs::vlp::base::PoolKey {
    ...
    fn clp_pair_info(amount_1: u128, amount_2: u128) -> euclid::token::PairWithDenomAndAmount {
    ...
    fn seed_clp_pool(deps: &mut crate::testing::helpers::MockDeps) {
        crate::state::POOL_KEY_TO_VLP
            .save(
                deps.as_mut().storage,
                clp_pool_key().to_map_key(),
                &"clp_vlp".to_string(),
            )
            .unwrap();
        seed_escrow(deps, "eth", "escrow_eth");
        seed_escrow(deps, "usdc", "escrow_usdc");
        set_escrow_token_allowed(deps, true);
    }

    /// Both legs zero is rejected: there is nothing to add.
    #[test]
    fn test_add_concentrated_liquidity_both_zero_amounts_fails() {
        let mut deps = mock_dependencies();
        init(&mut deps);
        seed_clp_pool(&mut deps);

        let user = deps.api.addr_make("user");
        let info = message_info(&user, &[]);

        let msg = ExecuteMsg::AddConcentratedLiquidity {
            pair_with_denom_and_amount: clp_pair_info(0, 0),
            pool_key: clp_pool_key(),
    ...
    /// A single zero leg is a one-sided position (range entirely on one side
    /// of the current price needs only that side's token): the request must
    /// pass the factory and dispatch the packet, funding only the nonzero leg.
    #[test]
    fn test_add_concentrated_liquidity_one_zero_leg_dispatches() {
        let mut deps = mock_dependencies();
        init(&mut deps);
        seed_clp_pool(&mut deps);
        deps.querier
            .bank
            .update_balance("any", vec![cosmwasm_std::coin(1_000_000, "ueth")]);
        deps.querier
            .bank
            .update_balance("any2", vec![cosmwasm_std::coin(1_000_000, "uusdc")]);

        let user = deps.api.addr_make("user");
        // Only the nonzero leg is funded; a zero leg must not require funds.
        let info = message_info(&user, &[cosmwasm_std::coin(1_000, "uusdc")]);

        let msg = ExecuteMsg::AddConcentratedLiquidity {
            pair_with_denom_and_amount: clp_pair_info(0, 1_000),
            pool_key: clp_pool_key(),
            lower_tick_index: -100,
            upper_tick_index: -10,
            position_id: None,
        ...
        // One submsg: the packet to the router. No transfer msgs for the zero leg.
        assert_eq!(res.messages.len(), 1);
        let entries: Vec<_> = PENDING_CONCENTRATED_ADD_LIQUIDITY
    ...
    // -----------------------------------------------------------------------
    // Execute: RequestPoolCreation – zero-amount voucher pair
    // -----------------------------------------------------------------------

    // Replicates the incident where a voucher/voucher request_pool_creation
    // with both token amounts set to 0 passed source-side validation and was
    // dispatched to the VSL, only failing there. The factory must reject zero
    // amounts before sending the packet. Vouchers skip the FundManager path
    // (which is what implicitly rejects zero for native/smart tokens), so the
    // guard has to be explicit.
    #[test]
    fn test_request_pool_creation_zero_amount_voucher_pair_fails() {
        let mut deps = mock_dependencies();
        init(&mut deps);
        ...
        let msg = ExecuteMsg::RequestPoolCreation {
            pair_with_denom_and_amount: pair_info,
            pool_config: euclid::msgs::vlp::base::PoolConfig::Stable {
