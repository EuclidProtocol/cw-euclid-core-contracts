...
// `true` once this factory has been registered with the router. The factory is
// the authority on duplicate registration: a second `RegisterFactory` for an
// already-registered factory is rejected here (the router then receives an error
// ack), so a duplicate or in-flight retry can never re-register or overwrite.
pub const REGISTERED: Item<bool> = Item::new("registered");
