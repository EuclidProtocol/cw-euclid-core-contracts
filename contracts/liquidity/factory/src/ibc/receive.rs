...
use euclid_ibc::wire::envelope::{factory::FactoryReceiveMsg, AcknowledgementMsg};
use euclid_ibc::wire::msgs::RegisterFactorySendMsg;
use euclid_ibc::wire::msgs::ReleaseEscrowSendMsg;

use crate::{
    reply::RELEASE_ESCROW_REPLY_ID,
    state::{REGISTERED, STATE, TOKEN_TO_ESCROW},
};
...
fn execute_register_router(
    ...
    // The factory is the authority on duplicate registration. If it has already
    // been registered, reject — the error propagates back to the router as an
    // error ack (relayed via `reply_always`, or surfaced through the native reply
    // path), so a duplicate or in-flight retry can never re-register or let a
    // second ack overwrite the first registration on the router.
    ensure!(
        !REGISTERED.load(deps.storage).unwrap_or(false),
        ContractError::new("Factory already registered")
    );
    REGISTERED.save(deps.storage, &true)?;
...
mod tests {
    ...
    fn release_escrow_msg(token_id: &str, recipient: &str, amount: u128) -> FactoryReceiveMsg {
        FactoryReceiveMsg::ReleaseEscrow(ReleaseEscrowSendMsg {
            sender: CrossChainUser {
    ...
    // -----------------------------------------------------------------------
    // execute_register_router — duplicate registration (factory is the authority)
    // -----------------------------------------------------------------------

    /// The factory tracks whether it has already been registered. A second
    /// `RegisterFactory` — a duplicate or an in-flight retry — is rejected, which
    /// the relay/native paths turn into an error ack back to the router, so the
    /// router never re-registers or overwrites the first registration.
    #[test]
    fn test_register_factory_twice_is_rejected() {
        ...
        // First registration succeeds and flips the REGISTERED flag.
        let msg = cosmos_register_msg(MOCK_CONTRACT_ADDRESS, MOCK_CHAIN_ID, TEST_CHAIN_UID);
        reusable_internal_call(&mut deps.as_mut(), env.clone(), msg).unwrap();
        assert!(REGISTERED.load(deps.as_ref().storage).unwrap());

        // Second registration for the same (already-registered) factory is rejected.
        let msg = cosmos_register_msg(MOCK_CONTRACT_ADDRESS, MOCK_CHAIN_ID, TEST_CHAIN_UID);
        let err = reusable_internal_call(&mut deps.as_mut(), env, msg).unwrap_err();
        assert_eq!(
            err,
            euclid::error::ContractError::new("Factory already registered")
        );
    }
    ...
    fn test_register_factory_cosmos_wrong_chain_id_returns_error() {
        ...
        assert_eq!(
            err,
            euclid::error::ContractError::new(&format!(
                "Factory chain ID mismatch: {} != {}",
                "wrong-chain-id", MOCK_CHAIN_ID
            ))
        );
    }
