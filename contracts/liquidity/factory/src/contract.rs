...
pub mod execute {
    use cosmwasm_std::{
        to_json_binary, CosmosMsg, DepsMut, Env, IbcMsg, IbcPacket, IbcTimeout, MessageInfo,
        Response, Uint128,
    };
    ...
    pub fn execute_request_pool_creation(
        ...
        if token_1.is_smart() {
            let msg =
                token_1.create_transfer_msg(token_1_reserve, env.contract.address.to_string());
            msgs.push(msg);
        }
        // DO same for token 2
        if token_2.is_smart() {
            let msg =
                token_2.create_transfer_msg(token_2_reserve, env.contract.address.to_string());
            msgs.push(msg);
        }
